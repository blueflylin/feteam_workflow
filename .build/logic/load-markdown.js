 ...
 */
define("logic/load-markdown", [ "$", "templates/template", "willkan/handlebars/1.0.0/handlebars.runtime", "utils/animationHelper", "modernizr", "static/post-package", "static/demo/backbone/demo", "underscore", "backbone", "willkan/backbone-localStorage/1.1.6/backbone.localStorage" ], function(require) {
    console.log("begin load md");
    var $ = require("$"), templates = require("templates/template");
    require("utils/animationHelper");
    $(function() {
        var $article = $("article"), $aside = $("aside"), $navBtn = $(".nav-btn"), small = $aside.css("display") === "none";
        if (!small) {
            $(document.head).append("<style>" + "aside.columns{position: fixed; left:" + $aside.offset().left + "px; visibility:visible;}</style>");
        } else {
            $(document.head).append("<style>aside.columns{position: fixed; right:0;}</style>");
            ...
            $(document.body).animationHelper({
                ...
                before: function(target, trigger) {
                    ...
                    if (hidden || trigger.parents("aside").length != 0) {
        ...
        var articleResult = "", asideResult = "", callbacks = [];
        //存放插件的回调函数
