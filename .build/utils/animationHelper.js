define("utils/animationHelper", [ "$", "modernizr" ], function(require) {
    var $ = require("$"), Modernizr = window.Modernizr || require("modernizr");
    $.extend($.fn, {
        animationHelper: function(options) {
            var $trigger = $(this), $animateTarget = $(options.animateTarget), eventType = options.eventType, selector = options.selector, after = options.after, during = options.during, before = options.before || true, bubble = options.bubble || false, isAnimating = false, animEndEventNames = {
            ...
            function init() {
                ...
                selector ? $trigger.on(eventType, selector, animate) : $trigger.on(eventType, animate);
                function animate(e) {
                    var $trigger = $(this);
                    if (isAnimating) {
                        return false;
                    ...
                    isAnimating = true;
                    $animateTarget.addClass($animateTarget.animateClass + " " + $animateTarget.finalClass).on(animEndEventName, function() {
